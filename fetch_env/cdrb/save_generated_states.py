...
def update_args(**kwargs):
    ...
    torch.cuda.manual_seed_all(Args.set_seed)
...
def save_traj(diffusion, episode_data):
    generated_to_save = []
    ...
    for each_run in range(Args.n_test):
        ...
        # env = gym.make(Args.dataset, render_mode='human')
        obs, info = env.reset()
        cond = {}
        if Args.render_mode == 'expert':
            pass
        ...
        goal = denormalize(goal)   
        generated_states = x_recon[0, :, Args.action_dim:Args.observation_dim+Args.action_dim]
        generated_states = generated_states.cpu().numpy()
        generated_states = denormalize(generated_states)

        generated_to_save.append({"states": generated_states.tolist()})


    dir_pth = f'../saved_trajectories/{Args.dataset}/'
    os.makedirs(dir_pth, exist_ok=True)
    saved_generated_path = dir_pth + Args.model_type + f'_seed_{Args.set_seed}_nickname_{Args.nickname}_epoch_{int(Args.n_train_steps/100)}.json'
    with open(saved_generated_path, 'w') as f:
        json.dump(generated_to_save, f)
