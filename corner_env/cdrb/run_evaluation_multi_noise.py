...
def run_episode(ep_idx, episode_data, predicted_data, normalize_constants, traj_exec, gen_id):
    log_dict = {}
    controller_imgs = {}
    ...
    log_dict[f"distances/gt_traj_{gen_id}"] = cumulative_distance_gt_traj
    log_dict[f"distances/pred_traj_{gen_id}"] = cumulative_distance_pred_traj
    log_dict[f"distance_ratios/pred_traj_{gen_id}"] = cumulative_distance_pred_traj / cumulative_distance_gt_traj
    ...
    for _traj_exec in traj_exec:
        ...
        log_dict[f"success_rate/{_traj_exec}_{gen_id}"] = int(_success)
        ...
        log_dict[f"controller_steps/{_traj_exec}_{gen_id}"] = len(executed_traj)
        log_dict[f"distances/{_traj_exec}_{gen_id}"] = cumulative_distance_exec
        log_dict[f"distance_ratios/{_traj_exec}_{gen_id}"] = cumulative_distance_exec / cumulative_distance_gt_traj
        if _success:
            log_dict[f'success_distance_ratios/{_traj_exec}_{gen_id}'] = cumulative_distance_exec / cumulative_distance_gt_traj
...
def evaluate_traj(episode_data, predicted_data, num_vis=10, traj_exec=['controller'], epoch=0, multi=16, gen_id=0):
    """
    Args:
    ...
    all_tasks = [
        executor.submit(
            run_episode,
            *(ep_idx, episode_data, predicted_data, normalize_constants, traj_exec, gen_id)
        ) for ep_idx in range(num_vis)
    ]
    ...
    wandb_log = {"num_episodes": num_vis, "epoch":epoch, f"video_{gen_id}": wandb.Video(video, fps=5, format="mp4")}
...
def evaluate(diffusion, selection_strategy, epoch, multi=16, gen_id=0):
    ...
    assert selection_strategy != 'from_env'
    evaluate_traj(eval_traj.cpu().numpy(), pred_traj[-1].cpu().numpy(), num_vis=Args.num_tests, traj_exec=Args.traj_exec, epoch=epoch, multi=multi, gen_id=gen_id)
...
if __name__ == '__main__':
    ...
    parser.add_argument('-n', '--num_gen', type=int, default=10)
    ...
    if args.epoch:
        for i, model_path in enumerate(files[1:]):
            ...
            for n in range(args.num_gen):
                print(f"gen id: {n}")
                evaluate(diffusion, selection_strategy='from_val_tail',epoch=i, multi=args.multi, gen_id=n)
    else:
        diffusion = prepare_model(files[0])
        for n in range(args.num_gen):
            print(f"gen id: {n}")
            evaluate(diffusion, selection_strategy='from_val_tail',epoch=(len(files)-2), multi=args.multi, gen_id=n)