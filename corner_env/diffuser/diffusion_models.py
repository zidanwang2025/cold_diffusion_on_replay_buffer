...
class GaussianDiffusion(nn.Module):
    def __init__(self, model, horizon, observation_dim, action_dim, n_timesteps=1000,
        ...
        # TODO: (TY) Look into the cosine beta schedule
        # At least we can compare max(betas) for n_timesteps=128 vs 1000
    ...
    def backward_sample_loop(self, shape, cond, verbose=False, cold=False):
        ...
        batch_size = shape[0]
        x = torch.randn(shape)
        x = apply_conditioning(x, cond, self.action_dim, cond_vels=Args.cond_vels).to(Args.device)
