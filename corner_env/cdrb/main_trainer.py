...
class Trainer(object):
    def __init__(
        ...
        temp_dataloader = torch.utils.data.DataLoader(
            self.dataset, batch_size=train_batch_size, num_workers=16, shuffle=True, pin_memory=True
        )
        ...
        temp_val_dataloader = torch.utils.data.DataLoader(
            self.val_dataset, batch_size=train_batch_size, num_workers=16, shuffle=True, pin_memory=True
        )

        temp = iter(temp_val_dataloader)
        self.n_val_batch = sum(1 for _ in temp) # maybe lower the number of batch for each validation
    ...
    def train(self, n_train_steps):
        ...
        for step in range(n_train_steps):
            ...
            wandb.log({'train_loss': train_loss, 'step': self.step})
            self.step += 1
