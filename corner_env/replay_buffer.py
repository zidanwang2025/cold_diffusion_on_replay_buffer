...
class ReplayBuffer(object):
    def __init__(self, state_dim, action_dim, dataset_name, max_size=int(1e6), trim_buffer_mode="kmeans"):
        self.state = None
        self.action = None
        self.reward = None
        self.terminal = None
    ...
    def get_k_means_centroid(self, k=100, file_path=None, joint_as=False):
        ...
        if joint_as:
            if file_path is None:
                # Special handling for TTIC side
                fname = '_'.join(('joint_k', str(k), self.dataset_name)) + '.pkl'
                if "LMN_OUTPUT_DIR" in os.environ:
                    assert os.path.isdir('/cdrb/replay')
                    os.makedirs("/cdrb/replay/replay_buffer_kmeans-20230603/", exist_ok=True)
                    file_path = "/cdrb/replay/replay_buffer_kmeans-20230603/" + fname
                else:
                    file_path = "replay_buffer_kmeans-20230603/" + fname
            if os.path.exists(file_path):
                with open(file_path, 'rb') as f:
                    action_state = pickle.load(f)
                self.action_state_kmeans = action_state
            else:
                print(f"Replay buffer file {file_path} does not exist.\nRunning k-means for joint action-state")
            ...
            if file_path is None:
                # Special handling for TTIC side
                fname = '_'.join(('k', str(k), self.dataset_name)) + '.pkl'
                if "LMN_OUTPUT_DIR" in os.environ:
                    # NOTE: mount: `/cdrb` -> `slurm:/share/data/ripl-takuma/projects/cdrb`
                    assert os.path.isdir('/cdrb/replay')
                    file_path = "/cdrb/replay/replay_buffer_kmeans-20230603/" + fname
                else:
                    file_path = "replay_buffer_kmeans-20230603/" + fname
    ...
    #         if joint_as:
    #             if file_path is None:
    #                 file_path = "replay_buffer_kmeans/joint_k_" + str(k) + ".pkl"
    #             if os.path.exists(file_path):
    #                 with open(file_path, 'rb') as f:
    #                     action_state = pickle.load(f)
    #                 self.action_state_kmeans = action_state
    ...
    #                 action_state = torch.cat((self.action, self.state), dim=1)
    #                 kmeans = KMeans(n_clusters=k).fit(np.array(action_state))
    #                 self.action_state_kmeans = torch.tensor(kmeans.cluster_centers_)
    #                 with open(file_path, 'wb') as f:
    #                     pickle.dump(self.action_state_kmeans, f)
    #         else:
    #             if file_path is None:
    #                 file_path = "replay_buffer_kmeans/k_" + str(k) + ".pkl"
    #             if os.path.exists(file_path):
    #                 with open(file_path, 'rb') as f:
    #                     action_state = pickle.load(f)
    #                 self.action_kmeans = action_state["action"]
    ...
    #             else:
    #                 kmeans = KMeans(n_clusters=k).fit(np.array(self.action))
    #                 self.action_kmeans = torch.tensor(kmeans.cluster_centers_)
    #                 kmeans = KMeans(n_clusters=k).fit(np.array(self.state))
    #                 self.state_kmeans = torch.tensor(kmeans.cluster_centers_)
    #                 action_state = {"action": self.action_kmeans, "state": self.state_kmeans}
    #                 with open(file_path, 'wb') as f:
    #                     pickle.dump(action_state, f)
    #         print("done")
    ...
    def sample_trajectory(self, batch_size):
        pass
        # ind = np.random.randint(0, len(self.start_end), size=batch_size)
        # start_end_index = self.start_end[ind]
        # trajectory_states = []
        # trajectory_actions = []
        # for index in start_end_index:
        #     trajectory_states.append(self.state[index[0]:index[1]])
        #     trajectory_actions.append(self.action[index[0]:index[1]])
        #
        # trajectory_states = copy.deepcopy(trajectory_states)
        # trajectory_actions = copy.deepcopy(trajectory_actions)
        #
        # return (
        #     trajectory_states,
        #     trajectory_actions,
    ...
    def convert_dict(self, dataset, file_path=None, joint_as=False, k=200, d=0.5):
        if file_path is None:
            self.state = torch.FloatTensor(dataset['observations'][:, :self.state_dim])
            self.action = torch.FloatTensor(dataset['actions'][:, :self.action_dim])
            self.reward = dataset['rewards']
            self.terminal = dataset['terminals']
            self.goal = dataset['goals']
        else:
